...
class BotGAT(nn.Module):
    def __init__(self, hidden_dim, des_size=768, tweet_size=768,user_size=768,num_prop_size=5, cat_prop_size=3, dropout=0.3):
        ...
        self.linear_relu_user = nn.Sequential(
            nn.Linear(user_size, hidden_dim // 4),
            nn.LeakyReLU()
        )
    ...
    nn.Linear(5 * (hidden_dim // 4), hidden_dim),  # 从 160 到 128
    nn.LeakyReLU()
)
    ...
    def forward(self, des, tweet, user, num_prop, cat_prop, edge_index, edge_type=None):
        # print("num_prop.shape:", num_prop.shape)
        d = self.linear_relu_des(des)
        t = self.linear_relu_tweet(tweet)
        n = self.linear_relu_num_prop(num_prop)
        c = self.linear_relu_cat_prop(cat_prop)
        u = self.linear_relu_user(user)
        x = torch.cat((d, t, n, c, u), dim=1)
        x = self.dropout(x)
