...
BASE_DIR = 'workspace/workspace/TwiBot-22/src/BotRGCN'
# def get_transfer_data():
#     path = osp.join(BASE_DIR, data_index[dataset_name], 'processed_data')
...
def get_train_data(dataset_name):
    print(f"[Debug] Received dataset name: {dataset_name}")
    path = osp.join(BASE_DIR, data_index[dataset_name], 'processed_data')
    if not osp.exists(path):
        raise KeyError
    labels = torch.load(osp.join(path, 'label.pt'))
    des_embedding = torch.load(osp.join(path, 'des_tensor.pt'))
    # tweet_embedding = torch.load(osp.join(path, 'user_embedding_new1.pt'))
    tweet_embedding = torch.load(osp.join(path, 'tweets_tensor.pt'))
...
# 整体分布
        print("Overall labels:", torch.unique(labels, return_counts=True))

# 训练/验证分布
        print("Train labels:", torch.unique(labels[train_idx], return_counts=True))
        print("Val   labels:", torch.unique(labels[val_idx],   return_counts=True))
    return Data(edge_index=edge_index,
                edge_type=edge_type,
                y=labels,
                des_embedding=des_embedding,
