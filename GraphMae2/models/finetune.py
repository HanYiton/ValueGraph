...
def finetune(
    ...
    def eval_with_lc(model, loader):
        ...
        with torch.no_grad():
            for batch in epoch_iter:
                ...
                preds.extend(prediction.argmax(1).cpu().numpy())
                labels.extend(batch_lbls.cpu().numpy())
    ...
    for epoch in range(max_epoch_f):
        ...
        if not full_graph_forward:
            print("val Acc {:.4}, Best Val Acc {:.4f}".format(val_acc, best_val_acc))
        else:
            print(
                "Val Acc {:.4x}, Best Val Acc {:.4f} Test Acc {:.4f}".format(
