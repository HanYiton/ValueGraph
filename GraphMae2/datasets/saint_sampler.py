...
class SAINTSampler:
    ...
    def __init__(
        ...
        if os.path.exists(graph_fn):
            ...
            t = time.perf_counter()
            for num_nodes, subgraphs_nids, subgraphs_eids in tqdm(
                loader, desc="preprocessing saint subgraphs"
            ):
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            ...
            subgraph_eids = dgl.node_subgraph(self.train_g, subgraph_nids).edata[
                dgl.EID
            ]
    ...
    def __compute_degree_norm(self):
        ...
        self.train_g.ndata["train_D_norm"] = (
...
class SAINTRandomWalkLoader(SAINTSampler):
    ...
    def __init__(self, feats, num_roots, length, **kwargs):
        ...
        self.feats = feats
        super(SAINTRandomWalkLoader, self).__init__(
...
def build_saint_dataloader(feats, graph, dataset_name, online=False, **kwargs):
    ...
    loader = DataLoader(
        saint_sampler, collate_fn=saint_sampler.__collate_fn__, batch_size=1, **kwargs
    )
    return loader
